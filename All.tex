...
\section*{Appendix}
\subsection*{Vessel data}
The vessel data is given by the sets summarized in Table~\ref{tab:sets} and the constants summarized in Table~\ref{tab:consts}. In the Vessel Stowage Model (VSM), buoyancy data and hydrostatic constraints are translated to sections' end points. The constants derived in this way are included in Table~\ref{tab:consts}. They are calculated from the vessel data as explained further below.
%
\begin{center}
\begin{table}[htbp]
...
\end{center}
%
The set of stations, $ST$, includes $\sigma^\trt{a}$ and $\sigma^\trt{f}$, which are two (artificial) stations at the aft most and fore most positions of the ship, respectively. Similarly, $F$ contains the frames $f^\trt{a}$ and $f^\trt{f}$. The areas (bounds) for these stations (frames) are trivial.
...
$W^\tau, W_b\in \mathbb{R}$ 																& The weight of container type $\tau\in T$, and of block $b\in B$, respectively. \\
$A_{\sigma, d}\in \mathbb{R}^+_0$ 													&	The area submerged in water at station $\sigma\in ST$ for a displacement of $d\in D$.\\
...
$A''_{p, d}\in \mathbb{R}^+_0$															&	Approximation of the area submerged in water at a longitudinal position $p$ for a displacement of $d\in \mathbb{R}$.\\
\end{tabular}
\caption{Constants used in the vessel data and derived constants.}\label{tab:consts}
\end{table}

\subsection*{Vessel Stowage Model}
The variables of the Vessel Stowage Model (VSM) are summarized in Table~\ref{tab:vars}. We note that even though $x_{l,\tau}$ is a number of containers and hence a natural number, we model it as a real number to ensure that the resulting model is an LP. The constraints of the model are described further below.
%
\begin{table}[hbtp]
\centering
...
\end{align}	 
%
The constraint in \eqref{typeDef} defines the variables, $x_\tau$, that specifies how many containers of a specific type $\tau$ is stowed on the vessel. These are the variables that the projected (in)equality system, the capacity model, is expressed in.
\begin{equation}
\label{typeDef}
...
\label{totalWeightDef}
	\forall s \in S:\quad w_s &= \sum_{\tau \in T} \sum_{l\in L_s} W_\tau\, x_{l,\tau} + xt_s + \sum_{b\in B} \mi{percent}([P^\trt{a}_b;P^\trt{f}_b],[P^\trt{a}_s;P^\trt{f}_s])\cdot W_b.\\
\label{totalDisp}
w & = \sum_{s\in S} w_s.
\end{align}
...
For the ballast tanks, we also require that each tank's content is within limits, and likewise for the the total displacement (weight). These constraints are given in \eqref{xPos}, \eqref{btLim}, and \eqref{wTotalLim} below.
\begin{align}
	\label{xPos}
	\forall l\in L, \tau\in T:&\quad 0 \leq x_{l,\tau}.\\
	%
	\label{btLim}
	\forall s \in S:	&\quad 0 \leq t_s \leq \sum_{b \in BT} \mi{percent}([P_b^\trt{a};P_b^\trt{f}],[P_s^\trt{a};P_s^\trt{f}])\cdot \mi{Max}^\trt{WT}_b\\
%
	\label{wTotalLim}
	&\quad w \leq \mi{Max}^\trt{W}
\end{align}
\paragraph{Hydrostatic constraints}
At each station $\sigma$ the submerged area of the cross-section for a given displacement $d\in D$ is given by $A_{\sigma,d}$. We make the assumption that above a given threshold $d_{min}\in D$ (and until the maximal displacement $d_{max} = \text{max}\{d\in D\}$), the submerged area is close to a linear function of the displacement. The submerged area $A'_{\sigma,d}$ for any $d\in [d_{min};d_{max}]$ at station $\sigma$ can therefore be approximated as 
\[
A'_{\sigma,d} = \frac{A_{\sigma,d_{max}} - A_{\sigma,d_{min}}}{d_{max} - d_{min}} \cdot (\mi{w} - d_{max}) + A_{\sigma,d_{max}},
\]
...
From this we can calculate the buoyancy of section $s \in S$ at a displacement $d\in \mathbb{R}$. First we let $P := \{P^\trt{a}_s, P^\trt{f}_s\} \cup \set{P_\sigma}{\sigma\in ST,\; P^\trt{a}_s < P_\sigma < P^\trt{f}_s}$ be the section's end points together with the stations between the two endpoints. Then the buoyancy of the section $s$ is given by
\[
...
\]
In a similar manner, we linearize the upper bound for the shear force between two consecutive frames, so that the upper bounds for the shear force at the aft endpoint of section $s$ is
\[
\mi{SF}^+_s = \frac{\mi{SF}^+_{f_2}-\mi{SF}^+_{f_1}}{P_{f_2}-P_{f_1}}\cdot(P^\trt{a}_s - P_{f_1}) + \mi{SF}^+_{f_1}, 
\]
where $f_1 = \mi{argmax}_{\set{f\in F}{P_f\leq P^\trt{a}_s}}P_f$ and $f_2 = \mi{argmax}_{\set{f\in F}{P_f>P^\trt{a}_s}}P_f$. Of course, if there is an $f\in F$ such that $P^\trt{a}_s = P_f$ then we let $\mi{SF}^+_s = \mi{SF}^+_f$. Similarly we find $\mi{SF}^-_s$, as well as $\mi{BM}^+_s$ and $\mi{BM}^\trt{-}_s$.   
...
\label{sf}
\forall s \in S:\;
\mi{sf}_s = \sum_{s' \in S'_s} (w_{s'} - b_{s',w}),\\
