...
%\subsection{Vessel description}\label{sec:vessel}
...
%
\subsection{Model}
The vessel data outlined above is used to construct the VSM described in the following.
%
\subsubsection{Sets and constants}
The vessel data given as input to the VSM is given by the sets summarized in Table~\ref{tab:sets} and the constants summarized in Table~\ref{tab:consts}. Buoyancy data and hydrostatic limits are translated to the sections' end points, and the derived constants are included in the table. The calculation of these are explained further below in Section~\ref{sec:constraints}.

\begin{table}[htbp]
\centering
\begin{tabular}{p{1.7cm}p{4.8cm}|p{1.7cm}p{4.8cm}}
...
\end{tabular}\caption{The sets used in the vessel data and the Vessel Stowage Model.}\label{tab:sets}
\end{table}
...
\multicolumn{2}{l}{\textbf{Derived constants}}\\
\hline\noalign{\smallskip}
...
\end{tabular}
\caption{Constants used in the vessel data and derived constants.}\label{tab:consts}
\end{table}

\subsubsection{Variables}
The variables of the Vessel Stowage Model (VSM) are summarized in Table~\ref{tab:vars}. As decision variables we use $x_{l,\tau}$ for all $l\in L$ and $\tau \in T$, which denotes the number of containers of type $\tau$ stowed in location $l$, and $t_s$ for all $s\in S$, which denotes the amount of ballast water in ballast tanks in section $s$. 
We note that even though $x_{l,\tau}$ is a number of containers and hence a natural number, we model it as a real number to ensure that the resulting model is an LP. 
%

As important auxiliary variables, we define $x_\tau$ for all $\tau\in T$ (see \eqref{typeDef} below), the number of containers of type $\tau$ on the vessel in total, disregarding their placements. It is the relationship among these variables that we are actually interested in, since these relationships define the wanted capacity model. Thus, when producing the capacity model from the VSM described in this section, we eliminate all variables but $\set{x_\tau}{\tau \in T}$.  
%
\begin{table}[hbtp]
\centering
...
%
\end{tabular}
\caption{Variables used in the Vessel Stowage Model.}\label{tab:vars}
\end{table}
%
...
\begin{align}
	...
	%
	\label{weight40Cap}
	\sum_{\tau \in T\setminus T^\trt{20}} W_\tau\cdot x_{l,\tau} \leq C_l^\trt{W40} 
			& \quad \forall l \in L. \\
	%
	\label{combinedWeightCap}
	0.5\cdot \sum_{\tau \in T^\trt{20}} W_\tau\cdot x_{l,\tau} + \sum_{\tau T\setminus \in T^\trt{20}		} W_\tau\cdot x_{l,\tau} \leq C_l^\trt{W40} 
			& \quad \forall l \in L.\\
	%
	\label{reeferSlotCap}
	\sum_{\tau \in T^\trt{R}} x_{l,\tau} \leq C_l^\trt{RS} 
			& \quad \forall l \in L. \\
	%
	\label{reeferCellCap}
	{\sum_{\tau \in T^\trt{R}\cap T^\trt{20}} 0.5\cdot x_{l,\tau} + \sum_{\tau \in T^\trt{R}\setminus T^\trt{20}} x_{l,\tau} \leq C_l^\trt{RC} } 
			& \quad \forall l \in L.
\end{align}	 
%
Constraint \eqref{typeDef} below defines the variables, $x_\tau$, that specifies how many containers of a specific type $\tau$ is stowed on the vessel. 
\begin{equation}
\label{typeDef}
	x_\tau = \sum_{l\in L} x_{l,\tau} \quad \forall{\tau \in T}.
\end{equation}
...
There is a small gap between two consecutive bays, and this space is divided evenly between two consecutive, cargo-containing sections. %, such that the aft end point of the fore-most section aligns with the fore end point of the aft-most section. 
...
\end{figure}
%
Using the end points of the sections, \eqref{totalWeightDef} below defines the total weight of section $s\in S$, including cargo, ballast tanks and the vessel itself. \eqref{totalDisp} defines the total displacement (weight) of the vessel.   

\begin{align}
\label{totalWeightDef}
	w_s &= \sum_{\tau \in T} \sum_{l\in L_s} W_\tau\, x_{l,\tau} + xt_s + \sum_{b\in B} \mi{percent}([P^\trt{a}_b;P^\trt{f}_b],[P^\trt{a}_s;P^\trt{f}_s])\cdot W_b \quad \forall s \in S.\\
\label{totalDisp}
w & = \sum_{s\in S} w_s.
\end{align}
Here, $\mi{percent}(J, I)$ defines how big a percentage of the interval $J=[J_1; J_2]$ lies within the interval $I = [I_1; I_2]$. That is, 
\[
\mi{percent}([J_1;J_2], [I_1; I_2]) = \frac{\max(\min(J_2, I_2)-\max(J_1, I_1),0)}{I_2-I_1}.
\]
%
Capacities for individual tanks are pooled into capacities for tanks within a section, such that the amount of ballast water in any section $s$ is limited by the amount of water in the portion of each tank that lies within that section combined \eqref{btLim}. Further it is required to be non-negative \eqref{btLim}. 
...
\begin{align}
	\label{btLim}
	0 &\leq t_s \leq \sum_{b \in BT} \mi{percent}([P_b^\trt{a};P_b^\trt{f}],[P_s^\trt{a};P_s^\trt{f}])\cdot \mi{Max}^\trt{WT}_b  \quad \forall s\in S.\\
	%
	\label{xPos}
	0 &\leq x_{l,\tau} \quad \forall l\in L, \tau\in T.\\
	%
	\label{wTotalLim}
	w &\leq \mi{Max}^\trt{W}. 
\end{align}
...
The submerged area between two consecutive stations $\sigma$ and $\sigma'$ are then linearized, so that the submerged area for displacement $d$ at any point $p\in[P_\sigma;P_{\sigma'}]$ %, where $\sigma\neq \sigma^\trt{f}$ and $\sigma' = \mi{argmin}_{\set{s}{P_s > P_\sigma}}P_s$, 
is approximated by
\[
A''_{p,d} = \frac{A'_{\sigma',d}-A'_{\sigma,d}}{P_{\sigma'}-P_\sigma}\cdot(p-P_\sigma) + A'_{\sigma,d}.
\]

From this we can calculate an approximation of the buoyancy of section $s\in S$ for a given $d\in \mathbb{R}^+_0$ by averaging the areas between the two endpoints of $s$ and multiplying with the distance between the points and the density of salt water.\footnote{Though all weights in principle have to be multiplied by the gravitational acceleration to get a force, it is a common practice to omit this in these calculations.} If there are stations lying within $s$, several volumes are calculated and added.
Thus, let $P := \{P^\trt{a}_s, P^\trt{f}_s\} \cup \set{P_\sigma}{\sigma\in ST,\; P^\trt{a}_s < P_\sigma < P^\trt{f}_s}$ be the section's end points together with the stations between the two endpoints. Then the buoyancy of section $s\in S$ at a displacement $d\in\mathbb{R}^+_0$ is defined by equation \eqref{defBancy}. 
\begin{equation}\label{defBancy}
...
\begin{gather}
	\label{sf}
	\mi{sf}_s = \sum_{s' \in S'_s} (w_{s'} - b_{s',w}) \quad \forall s \in S,\\
...
\\
	%
	\label{sfLim}
	\mi{SF}^-_s \leq \mi{sf}_s \leq \mi{SF}^+_s\quad \forall s \in S.\\
	%
	\label{bm}
	\mi{bm}_s = 	\sum_{s' \in S'_s} (w_{s'} + b_{s',w})\cdot |P^\trt{a}_s - \frac{P^\trt{f}_{s'}-P^\trt{a}_{s'}}{2}|
	\quad \forall s \in S.\\
	%
	\label{bmLim}
	\mi{BM}^-_s \leq \mi{bm}_s \leq \mi{BM}^+_s \quad \forall s \in S. 
\end{gather}
...
Previous FME frameworks : REWRITE
