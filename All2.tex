...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Background}
\red{Ja hvad gør vi i dette afsnit? Noget med data. Men det er ikke data vi beskriver, det er jo ``baggrund''.}
%In this section, we will describe the vessel stowage model (VSM), which will be projected to obtain the wanted capacity model. The model is adapted from work by Delgado (\cite{AlbertosThesis}). In the following, we will describe the given vessel data and further explain how this information is translated into the VSM.
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Vessel Stowage Model}
This section presents the Vessel Stowage Model (VSM), which will be projected to obtain the wanted capacity model. The model is adapted from work by Delgado (\cite{AlbertosThesis}). It relies on vessel from a real vessel profile used by a professional loading computer.   %In the following, we will describe the given vessel data and further explain how this information is translated into the VSM.
%The data used as input to the VSMs are real vessel profile files used by a professional loading computer. 
...
%The vessel data given as input to the VSM is given by the sets summarized in Table~\ref{tab:sets} and the constants summarized in Table~\ref{tab:consts}. 
...
%Constraints
With these, the constraints of our VSM can be formulated. Note that there is no objective value since we for our task at hand only are interested in the feasible area of the model.
\begin{align}
	\label{20Cap}
...
Constraint \eqref{typeDef} defines the variables, $x_\tau$, that specifies how many containers of a specific type $\tau$ is stowed on the vessel. 
...
Here, $\mi{PR}_{b,s}$ defines the percentage of tank $b$ %the interval $J=[J_1; J_2]$ 
that lies within section $s\in S$, that is %the interval $I = [I_1; I_2]$. That is, 
\[
\mi{PR}_{b,s} = \frac{\max(\min(P^\texttt{f}_b, P^\texttt{f}_s)-\max(P^\texttt{a}_b, P^\texttt{a}_s),0)}{P^\texttt{a}_s-P^\texttt{f}_s}.
\]

Capacities for individual tanks are pooled into capacities for tanks within a section, such that the amount of ballast water in any section $s$ is limited by the amount of water in the portion of each tank that lies within that section combined, and further it is required to be non-negative \eqref{btLim}. 
...
%\subsubsubsection{Fourier-Motzkin-elimination}
...
\\\\
%\subsubsubsection{Gauss-elimination}
...
%\subsubsubsection{Parallel redundancy checking}
...
%\subsubsubsection{Boundary coarsening} 
...
%\subsubsection{Separating blocks}
...
%\subsubsection{Further decomposition}
