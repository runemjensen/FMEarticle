...
\\
\\
Using our previously described projection framework, we obtain the projection of $S$ w.r.t. $Y$ by calling \Call{ProjectNode}{root of $T$}, where $T$ is the tree structure constructed from $S$ and $Y$, and \Call{ProjectNode}{} is as below in Algorithm~\ref{alg:decomp}. We note that due to the boundary coarsening this only approximates $\mi{proj}_Y(S)$.
...
\Function{ProjectSingleNode}{Node $n$}
	...
	\State $n_{proj}\gets$ \Call{FME-Framework}{$S'$,$Y'$}
